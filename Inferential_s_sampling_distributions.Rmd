...
# type your code for this Exercise here, and Run Document
sample_means15_price <- ames %>%
  rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>%
  summarise(x_bar = mean(price))
```

```{r sum}
sample_means15_price %>%
  summarise(mean = mean(x_bar))
```

```{r }
ggplot(data = sample_means15_price, aes(x = x_bar)) +
...
# type your code for this Exercise here, and Run Document
sample_means150_2_price <- ames %>%
  rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>%
  summarise(x_bar = mean(price))
```

```{r sum}
sample_means150_2_price %>%
  summarise(mean = mean(x_bar))
```

```{r }
ggplot(data = sample_means150_2_price, aes(x = x_bar)) +
  geom_histogram(binwidth = 1000)
```
<div class="question">
 Which of the following is false? 
