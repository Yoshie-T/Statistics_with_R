...
outliers_k4 <- Bayes.outlier(m_lwage_iq, k = 4.034)
```

```{r }
outliers_df_k4 <- data.frame(probability = outliers_k4$prob.outlier,
                          case = 1:length(outliers_k4$prob.outlier))
```
...
m_wage_race <- lm(wage ~ black, data = wage)
```
...
HPM_pred_lwage <- predict(bma_lwage, estimator = "HPM")
ci_hpm_lwage <- confint(HPM_pred_lwage, estimator = "HPM")
opt_HPM <- which.max(HPM_pred_lwage$fit)
exp(ci_hpm_lwage[opt_HPM, ])
```
