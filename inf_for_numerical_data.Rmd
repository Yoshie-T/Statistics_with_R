...
habit_bp <<-ggplot(data=nc, aes(y= weight, x=habit ) ) # Creates boxplots
habit_bp <- habit_bp + geom_boxplot() 
habit_bp <- habit_bp + ggtitle("Baby's weight per m habit") # Adds a title
habit_bp <- habit_bp +  ylab("weight") + xlab("Mother's habit") # Adds kaveks
habit_bp # displays the boxplots
```
...
```{r inf-weight-habit-ht, tidy=FALSE}
inference(y = weight, x = habit, data = nc, statistic = "mean", type = "ci", null = 0, method = "theoretical")
```
...
inference(y = weeks, data = nc, statistic = "mean", type = "ci", conf_level = 0.99, null = 0, method = "theoretical")
```
