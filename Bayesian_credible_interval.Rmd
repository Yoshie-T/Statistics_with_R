...
First, let us do some exercises to learn how to use this app. (Note: if 
the app below doesn't display properly when you Run the Document, try 
running the code chunk individually by clicking on the green arrow.)
...
```{r }
pbeta(q = 0.133, shape1 = 2, shape2 = 6)
```

```{r }
prior = c(0.5, 0.5)
likelihood = c(dbinom(2, 2, 0.75), dbinom(2, 2, 0.5))
posterior = prior * likelihood / sum(prior * likelihood)
post.cond.head = c(dbinom(1, 1, 0.75), dbinom(1, 1, 0.5))
post.head = sum(posterior * post.cond.head)
post.head
```