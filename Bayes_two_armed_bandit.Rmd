...
and the other to be the "bad" machine. (Note: if the app below doesn't display 
properly when you Run the Document, try running the code chunk individually by 
...
data11 <- data.frame(machine = c(1), outcome = c("W"))
data12 <- data.frame(machine = c(1), outcome = c("W"))
data13 <- data.frame(machine = c(2), outcome = c("W"))
data14 <- data.frame(machine = c(2), outcome = c("W"))
data15 <- data.frame(machine = c(2), outcome = c("L"))

# Use piping and bandit_posterior to calculuate the posterior probabilities after each play
bandit_posterior(data11) %>% 
  bandit_posterior(data12, prior = .) %>% 
  bandit_posterior(data13, prior = .) %>% 
  bandit_posterior(data14, prior = .) %>% 
  bandit_posterior(data15, prior = .)
```
...
data21 <- data.frame(machine = c(2), outcome = c("W"))
data22 <- data.frame(machine = c(2), outcome = c("W"))
data23 <- data.frame(machine = c(2), outcome = c("L"))
data24 <- data.frame(machine = c(1), outcome = c("W"))
data25 <- data.frame(machine = c(1), outcome = c("W"))

# Use piping and bandit_posterior to calculuate the posterior probabilities after each play
bandit_posterior(data21) %>% 
  bandit_posterior(data22, prior = .) %>% 
  bandit_posterior(data23, prior = .) %>% 
  bandit_posterior(data24, prior = .) %>% 
  bandit_posterior(data25, prior = .)
```
